...
export function DisposalFeeFormV7({ initialFee, onSave, onCancel }: DisposalFeeFormProps) {
  // Form state
  ...
  const [allDisposalSiteRates, setAllDisposalSiteRates] = useState(false)
  const [allMinCharges, setAllMinCharges] = useState(false)
        ...
        initialFee.materialPricing.forEach((mp) => {
          if (mp.materialType) {
            pricing[mp.materialType] = {
              ...
              isDisposalSiteRate: mp.isDisposalSiteRate,
            }
          }
        })
      } else {
  ...
  const handleMaterialPricingChange = (
    ...
    setMaterialPricing((prev) => {
      ...
      // If auto-calculate is on and either rate or freeTonnage changes, update min charge
      if (autoCalculateMinCharge && (field === 'rate' || field === 'freeTonnage')) {
        const materialData = newPricing[material]
        if (materialData.rate && materialData.freeTonnage) {
          newPricing[material].minCharge = calculateMinCharge(
            materialData.freeTonnage,
            materialData.rate
          )
        }
      }
  ...
  const handleSelectAllDisposalSiteRates = (checked: boolean) => {
    setAllDisposalSiteRates(checked)
    setMaterialPricing((prev) => {
      const newPricing = { ...prev }
      selectedMaterials.forEach((material) => {
        newPricing[material] = {
          ...newPricing[material],
          isDisposalSiteRate: checked
        }
      })
      return newPricing
    })
  ...
  // Add effect to update allDisposalSiteRates when individual checkboxes change
  useEffect(() => {
    if (selectedMaterials.length > 0) {
      const allChecked = selectedMaterials.every(
        (material) => materialPricing[material]?.isDisposalSiteRate
      )
      setAllDisposalSiteRates(allChecked)
    }
  }, [materialPricing, selectedMaterials])

  // Add function to handle select all min charges
  const handleSelectAllMinCharges = (checked: boolean) => {
    setAllMinCharges(checked)
    setMaterialPricing((prev) => {
      const newPricing = { ...prev }
      selectedMaterials.forEach((material) => {
        newPricing[material] = {
  ...
  // Add effect to update allMinCharges when individual toggles change
  useEffect(() => {
    if (selectedMaterials.length > 0) {
      const allChecked = selectedMaterials.every(
        (material) => materialPricing[material]?.chargeable
      )
      setAllMinCharges(allChecked)
    }
  }, [materialPricing, selectedMaterials])
  ...
  const handleMaterialToggle = (material: string) => {
    setSelectedMaterials((prev) => {
      if (prev.includes(material)) {
        ...
        // Remove material pricing when material is deselected
        setMaterialPricing((prev) => {
          const { [material]: removed, ...rest } = prev
          return rest
        })
        return newMaterials
      } else {
        ...
        // Initialize material pricing with default values
        setMaterialPricing((prev) => ({
          ...prev,
          [material]: {
    ...
    if (Object.keys(newErrors).length === 0) {
      ...
      if (selectedMaterials.length > 0) {
        finalData.materialPricing = selectedMaterials.map((material) => ({
          ...
          isDisposalSiteRate: materialPricing[material]?.isDisposalSiteRate || false,
        }))
      }
  ...
  return (
    <div className="">
      <div className="p-6">
        <h2 className="text-xl font-bold">Disposal Fee Template v07</h2>
        <div>
            <ul className="list-disc list-inside px-2 text-sm">
                <li>adding disposal site rate options
                </li>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4">
              ...
              <div className="col-span-2 flex items-end gap-4">
                <div className="w-1/3">
                  ...
                  <RadioGroup
                    value={formData.rateStructure === "Per Container" ? "container" : "unit"}
                    onValueChange={(value) => {
                      if (value === "container") {
                        handleChange("rateStructure", "Per Container")
                      } else {
                        // Reset to a default unit of measure when switching to unit-based
                        handleChange("rateStructure", "Per Ton")
                      }
                    }}
              ...
              {formData.rateStructure !== "Per Container" && (
                <div className="col-span-2 flex items-end gap-4 mt-4">
                  {/* Unit of measure and overage fields moved to materials table */}
                </div>
              )}
            </div>
          </CardContent>
                      ...
                      <div className="space-y-4">
                        <div className="space-y-2">
                          <Table>
                            <TableHeader>
                              <TableRow>
                                <TableHead className="w-[120px]"><span className="text-xs">Material</span></TableHead>
                                <TableHead className="w-[80px]"><span className="text-xs">Unit of Measure</span></TableHead>
                                <TableHead className="w-[120px]">
                                  <div className="flex items-center justify-end pr-4">
                                    <div className="flex items-center space-x-2">
                                      <span className="text-xs">Disposal Site Rate</span>
                                        ...
                                        <TooltipProvider delayDuration={100}>
                                          <Tooltip>
                                            ...
                                            <TooltipContent side="right" className="max-w-[300px] p-3">
                                              <p className="font-medium mb-1">Disposal Site Rate</p>
                                              <p className="text-sm text-muted-foreground">
                                                Enable to use the disposal site's rate for this material. When enabled, 
                                                the rate field will be disabled as the rate will be determined by the 
                                                disposal site.
                                              </p>
                                            </TooltipContent>
                                          </Tooltip>
                                ...
                                <TableHead className="w-[80px]"><span className="text-xs">Rate</span></TableHead>
                                <TableHead className="w-[80px]"><span className="text-xs">Included Amount</span></TableHead>
                                <TableHead className="w-[120px]">
                                  <div className="w-[100] flex items-center space-x-2">
                                    <span className="text-xs">Min. Charge</span>
                                    <div className="flex items-center space-x-1">
                                      <TooltipProvider delayDuration={100}>
                                        <Tooltip>
                                          <TooltipTrigger asChild>
                                          ...
                                          <TooltipContent side="right" className="max-w-[300px] p-3">
                                            ...
                                            <p className="text-sm text-muted-foreground">
                                              ...
                                              Amount × Rate</span>). Override default by entering a custom value. 
                                      ...
                                      <Switch
                                        ...
                                        checked={allMinCharges}
                                        onCheckedChange={handleSelectAllMinCharges}
                                        className="h-4 w-8"
                                      />
                                    </div>
                                  </div>
                                </TableHead>
                                <TableHead className="w-[120px]"><span className="text-xs">Overage Threshold</span></TableHead>
                                <TableHead className="w-[120px]"><span className="text-xs">Overage Fee</span></TableHead>
                              </TableRow>
                            </TableHeader>
                            <TableBody>
                                ...
                                return (
                                  <TableRow key={material.id}>
                                    ...
                                    <TableCell className="w-[120px]">
                                      <Select
                                        value={materialPricing[material.name]?.rateStructure || "Per Ton"}
                                        onValueChange={(value) => handleMaterialPricingChange(material.name, "rateStructure", value)}
                                      >
                                        <SelectTrigger className="h-8 text-xs">
                                          <SelectValue placeholder="Select unit" />
                                        </SelectTrigger>
                                        <SelectContent>
                                          <SelectItem value="Per Ton" className="text-xs">Tons</SelectItem>
                                          <SelectItem value="Per Gallon" className="text-xs">Gallons</SelectItem>
                                          <SelectItem value="Per Item" className="text-xs">Items</SelectItem>
                                          <SelectItem value="Per Pound" className="text-xs">Pounds</SelectItem>
                                          <SelectItem value="Per Yard" className="text-xs">Yards</SelectItem>
                                        </SelectContent>
                                      </Select>
                                    </TableCell>
                                    <TableCell className="w-[120px]">
                                      ...
                                      <div className="relative flex items-center gap-2">
                                        <div className="relative w-[140px]">
                                          ...
                                          <Input
                                            ...
                                            className={cn(
                                              "pl-7 pr-10 h-8 text-xs",
                                              materialPricing[material.name]?.isDisposalSiteRate && "opacity-50 cursor-not-allowed"
                                            )}
                                            placeholder={(materialPricing[material.name]?.rateStructure && 
                                              feeStructureConfigs[materialPricing[material.name].rateStructure]?.rateConfig.placeholder) ?? "0.00"}
                                            step={(materialPricing[material.name]?.rateStructure && 
                                              feeStructureConfigs[materialPricing[material.name].rateStructure]?.rateConfig.step) ?? "0.01"}
                                            disabled={materialPricing[material.name]?.isDisposalSiteRate}
                                          />
                                          <span className="absolute right-3 top-2 text-muted-foreground text-xs">
                                            / {(materialPricing[material.name]?.rateStructure && 
                                              feeStructureConfigs[materialPricing[material.name].rateStructure]?.unitLabel.slice(0, -1)) ?? "unit"}
                                          </span>
                                        </div>
                                        {index === 0 && selectedMaterials.length > 1 && (
                                          <TooltipProvider delayDuration={100}>
                                            <Tooltip>
                                              ...
                                              <TooltipContent>
                                                <p className="text-xs">Copy this value to all materials</p>
                                              </TooltipContent>
                                            </Tooltip>
                                          </TooltipProvider>
                                        )}
                                      ...
                                      <div className="relative flex items-center gap-2">
                                        <div className="relative w-[120px]">
                                          <Input
                                            ...
                                            min="0"
                                            step="1"
                                            value={materialPricing[material.name]?.freeTonnage ?? ""}
                                            onChange={(e) => handleMaterialPricingChange(material.name, "freeTonnage", Number.parseInt(e.target.value) || 0)}
                                            className="pr-10 h-8 text-xs"
                                            placeholder="0"
                                          />
                                          <span className="absolute right-3 top-2 text-muted-foreground text-xs">
                                            {(materialPricing[material.name]?.rateStructure && 
                                              feeStructureConfigs[materialPricing[material.name].rateStructure]?.unitLabel) ?? "units"}
                                          </span>
                                        </div>
                                        {index === 0 && selectedMaterials.length > 1 && (
                                          <TooltipProvider delayDuration={100}>
                                              ...
                                              <TooltipTrigger asChild>
                                                <Button
                                                  ...
                                                  onClick={() => {
                                                    if (selectedMaterials.length > 0) {
                                                      const firstMaterial = selectedMaterials[0]
                                                      const firstMaterialIncludedTonnage = materialPricing[firstMaterial]?.freeTonnage || 0
                                                      const newPricing = { ...materialPricing }
                                                      selectedMaterials.forEach(material => {
                                                        newPricing[material] = {
                                                          ...newPricing[material],
                                                          freeTonnage: firstMaterialIncludedTonnage
                                                        }
                                                      })
                                                      setMaterialPricing(newPricing)
                                                    }
                                      ...
                                      <div className="relative flex items-center gap-2">
                                        <div className="relative w-[120px]">
                                          ...
                                          <Input
                                            ...
                                            className={cn(
                                              "pl-7 h-8 text-xs",
                                              !materialPricing[material.name]?.chargeable && "opacity-50 cursor-not-allowed"
                                            )}
                                            placeholder={materialPricing[material.name]?.rateStructure === "Per Gallon" ? "0.00" : 
                                              (materialPricing[material.name]?.rateStructure && 
                                              feeStructureConfigs[materialPricing[material.name].rateStructure]?.rateConfig.placeholder) ?? "0.00"}
                                            disabled={!materialPricing[material.name]?.chargeable}
                                          />
                                        </div>
                                        <div className="flex items-center space-x-1">
                                          <Switch
                                            id={`${material.name}-min-charge-toggle`}
                                            checked={materialPricing[material.name]?.chargeable || false}
                                            onCheckedChange={(checked) => 
                                              handleMaterialPricingChange(material.name, "chargeable", checked)
                                            }
                                            className="h-4 w-8"
                                          />
                                        </div>
                                      ...
                                      <div className="relative flex items-center gap-2">
                                        <div className="relative w-[120px]">
                                          <Input
                                            ...
                                            type="number"
                                            min="0"
                                            step={materialPricing[material.name]?.rateStructure === "Per Gallon" ? "0.0001" : "0.1"}
                                              ...
                                              <TooltipTrigger asChild>
                                                <Button
                                                  ...
                                                  onClick={() => {
                                                    if (selectedMaterials.length > 0) {
                                                      const firstMaterial = selectedMaterials[0]
                                                      const firstMaterialOverageThreshold = materialPricing[firstMaterial]?.overageThreshold || 0
                                                      const newPricing = { ...materialPricing }
                                                      selectedMaterials.forEach(material => {
                                                        newPricing[material] = {
                                                          ...newPricing[material],
                                                          overageThreshold: firstMaterialOverageThreshold
                                                        }
                                                      })
                                                      setMaterialPricing(newPricing)
                                                    }
                                      ...
                                      <div className="relative flex items-center gap-2">
                                        <div className="relative w-[120px]">
                                          ...
                                          <Input
                                            ...
                                            className="pl-7 pr-10 h-8 text-xs"
                                            placeholder={(materialPricing[material.name]?.rateStructure && 
                                              feeStructureConfigs[materialPricing[material.name].rateStructure]?.rateConfig.placeholder) ?? "0.00"}
                                            step={(materialPricing[material.name]?.rateStructure && 
                                              feeStructureConfigs[materialPricing[material.name].rateStructure]?.rateConfig.step) ?? "0.01"}
